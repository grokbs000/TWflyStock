...
export function registerOAuthRoutes(app: any) {
  app.get("/api/oauth/callback", async (req: any, res: any) => {
    ...
    if (!code || !state) {
      (res.status(400) as any).json({ error: "code and state are required" });
      return;
    }
    ...
    try {
      ...
      if (!userInfo.openId) {
        (res.status(400) as any).json({ error: "openId missing from user info" });
        return;
      }
      ...
      (res.status(500) as any).json({ error: "OAuth callback failed" });
    }
  });
}
